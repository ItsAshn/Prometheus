...
export const VideoUpload = component$(() => {
  ...
  const titleAutoGenerated = useSignal(false);

  const generateTitleFromFileName = $(async (fileName: string) =>
  ...
  const handleFileSelect = $(async (event: Event) => {
    ...
    if (file) {
      ...
      if (!title.value.trim() || titleAutoGenerated.value) {
        const autoTitle = await generateTitleFromFileName(file.name);
        if (autoTitle) {
  ...
  const handleUpload = $(async () => {
    ...
    try {
      ...
      if (assembleResult.success) {
        ...
        message.value =
          "Video uploaded successfully! Processing has started and the library will refresh when it is ready.";
